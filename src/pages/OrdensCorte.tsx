...
export default function OrdensCorte() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setEnriched(ordens.map((o) => ({ ...o, produtos: prodByOrdem.get(o.id) ?? [], grade: gradeByOrdem.get(o.id) ?? [] })));
    });
  }, [ordens]);
  ...
  const printOrdem = (o: OrdemCorteEnriched) => {
    ...
    printHTML(`Ordem de Corte - ${o.numero_oc}`, `
      <div class="header"><div><h1>Ordem de Corte — ${o.numero_oc}</h1><div class="subtitle">Ficha de produção</div></div><div class="company"><img src="/images/logo.png" class="logo" alt="MC" /><br/>Gestão - Mariana Cardoso</div></div>
      ...
      <div class="section"><div class="section-title">Grade de Tamanhos</div>
        <table><thead><tr><th>Tamanho</th><th>Quantidade</th></tr></thead>
        <tbody>${gradeRows}<tr class="total-row"><td>Total</td><td>${total}</td></tr></tbody></table>
      </div>
    `);
  };

  const openEdit = (o: OrdemCorteEnriched) => {
    setEditOrdem(o); setEditStatus(o.status ?? "Planejada"); setEditMetragem(o.metragem_risco); setEditFolhas(o.quantidade_folhas ?? 0); setEditOpen(true);
  };

  const handleEdit = async () => {
    if (!editOrdem) return;
    try {
      const { error } = await supabase.from("ordens_corte").update({ status: editStatus, metragem_risco: editMetragem, quantidade_folhas: editFolhas }).eq("id", editOrdem.id);
      if (error) throw error;
      toast.success("Ordem atualizada!"); setEditOpen(false); refetch();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success("Ordem excluída!"); setDeleteOpen(false); setDeleteId(null); refetch();
    } catch (e: any) { toast.error(e.message); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => navigate("/ordens-corte/nova")} className="gap-2"><Plus className="h-4 w-4" /> Nova Ordem</Button>
      </div>
            ...
            <motion.div key={o.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
              <Card className="hover:shadow-md transition-shadow">
                <CardContent className="pt-5 space-y-3">
                  ...
                  <div className="text-sm text-muted-foreground">{o.produtos.map((p) => p.nome_produto).join(", ") || "—"}</div>
                  <div className="flex flex-wrap gap-1.5">
                    {o.grade.map((g, j) => (
                      <span key={j} className="text-xs bg-muted px-2 py-1 rounded-md text-muted-foreground">{g.tamanho}: {g.quantidade}</span>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-2 pt-2 border-t border-border">
                    <Button variant="outline" size="sm" className="gap-1.5 flex-1" onClick={() => openEdit(o)}><Pencil className="h-3.5 w-3.5" /> Editar</Button>
                    <Button variant="outline" size="sm" className="gap-1.5" onClick={() => printOrdem(o)}><Printer className="h-3.5 w-3.5" /></Button>
                    <Button variant="outline" size="sm" className="gap-1.5 flex-1 text-destructive hover:text-destructive" onClick={() => { setDeleteId(o.id); setDeleteOpen(true); }}><Trash2 className="h-3.5 w-3.5" /> Excluir</Button>
                  </div>
                </CardContent>
              </Card>
            </motion.div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Select value={editStatus} onValueChange={setEditStatus}>
                ...
                <SelectContent>
                  {["Planejada", "Em Corte", "Finalizada"].map((s) => (<SelectItem key={s} value={s}>{s}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
