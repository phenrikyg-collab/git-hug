...
export default function Metas() {
  ...
  const handleCreate = async () => {
    try {
      await createMut.mutateAsync({ mes: mes + "-01", meta_mensal: metaMensal, dias_uteis: diasUteis, meta_ticket_medio: ticketMedio });
      toast.success("Meta criada!"); setOpen(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard title="Meta do Mês" value={formatCurrency(d?.meta_mensal)} subtitle={`Vendido: ${formatCurrency(d?.vendido)}`} icon={Target} variant="primary" />
        <StatCard title="Projeção Faturamento" value={formatCurrency(r?.media_real_diaria && r?.dias_restantes ? (r.faturamento_realizado ?? 0) + r.media_real_diaria * r.dias_restantes : 0)} subtitle={`Média diária: ${formatCurrency(r?.media_real_diaria)}`} icon={TrendingUp} variant="default" />
        <StatCard title="Risco da Meta" value={r?.nivel_risco ?? "—"} subtitle={`Necessário/dia: ${formatCurrency(r?.media_necessaria_diaria)}`} icon={AlertTriangle} variant={r?.nivel_risco === "alto" ? "danger" : r?.nivel_risco === "moderado" ? "warning" : "success"} />
      </div>
              ...
              <TableBody>
                {metas?.map((m) => (
                  <TableRow key={m.id}>
                    ...
                    <TableCell className="text-right">{m.meta_mensal && m.dias_uteis ? formatCurrency(m.meta_mensal / m.dias_uteis) : "—"}</TableCell>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2"><Label>Meta Mensal (R$)</Label><Input type="number" step="0.01" value={metaMensal} onChange={(e) => setMetaMensal(Number(e.target.value))} /></div>
            <div className="space-y-2"><Label>Dias Úteis</Label><Input type="number" value={diasUteis} onChange={(e) => setDiasUteis(Number(e.target.value))} /></div>
            <div className="space-y-2"><Label>Ticket Médio Meta (R$)</Label><Input type="number" step="0.01" value={ticketMedio} onChange={(e) => setTicketMedio(Number(e.target.value))} /></div>
