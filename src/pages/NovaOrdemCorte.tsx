...
export default function NovaOrdemCorte() {
  ...
  const setGradeForCor = (corKey: string, tamanho: string, qty: number) => {
    setGradeMultiCor((prev) => ({ ...prev, [corKey]: { ...(prev[corKey] ?? {}), [tamanho]: qty } }));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const rolosItems = Array.from(selectedRolos).map((rolo_id) => ({ rolo_id, metragem_utilizada: metrosRolo[rolo_id] ?? 0 }));
      const allTamanhos = [...new Set(gradeItems.map((g) => g.tamanho))];
      await createMut.mutateAsync({
        ordem: { numero_oc: numeroOC, grade_tamanhos: allTamanhos, metragem_risco: metrosRisco, quantidade_folhas: folhas, status: "Planejada" },
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <h1 className="text-3xl font-serif font-bold text-foreground">Nova <span className="text-primary">Ordem de Corte</span></h1>

      <Card>
        <CardContent className="pt-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2"><Label>Número da OC</Label><Input value={numeroOC} readOnly className="bg-muted" /></div>
            <div className="space-y-2"><Label>Metros do Risco</Label><Input type="number" step="0.01" value={metrosRisco} onChange={(e) => setMetrosRisco(Number(e.target.value))} /></div>
            <div className="space-y-2"><Label>Quantidade de Folhas</Label><Input type="number" value={folhas} onChange={(e) => setFolhas(Number(e.target.value))} /></div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select value={produtoId} onValueChange={setProdutoId}>
              ...
              <SelectContent>
                {produtos?.filter((p) => p.ativo).map((p) => (<SelectItem key={p.id} value={p.id}>{p.nome_do_produto}</SelectItem>))}
              </SelectContent>
            </Select>
            {produtoSelecionado && (
                  ...
                  return (
                    <div key={r.id} className="flex items-center gap-3 p-2 rounded-lg border border-border">
                      ...
                      {selectedRolos.has(r.id) && (
                        <Input type="number" step="0.01" className="w-24" placeholder="Metros" max={r.metragem_disponivel ?? 0}
                          value={metrosRolo[r.id] ?? ""} onChange={(e) => setMetrosRolo({ ...metrosRolo, [r.id]: Number(e.target.value) })} />
                      )}
                    </div>
                  );
                })
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
                    {TAMANHOS.map((t) => (
                      <div key={t} className="space-y-1">
                        ...
                        <Input type="number" min={0} value={gradeMultiCor[corKey]?.[t] ?? ""} onChange={(e) => setGradeForCor(corKey, t, Number(e.target.value))} placeholder="0" />
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="p-4 rounded-lg bg-muted/50 border border-border grid grid-cols-3 gap-4">
            <div><p className="text-xs text-muted-foreground">Total de Peças</p><p className="text-xl font-serif font-bold text-foreground">{totalPecas}</p></div>
            <div><p className="text-xs text-muted-foreground">Consumo Total</p><p className="text-xl font-serif font-bold text-foreground">{consumoTotal.toFixed(2)}m</p></div>
            <div><p className="text-xs text-muted-foreground">Metros Alocados</p><p className={`text-xl font-serif font-bold ${estoqueInsuficiente ? "text-destructive" : "text-foreground"}`}>{metrosAlocados.toFixed(2)}m</p></div>
          </div>
