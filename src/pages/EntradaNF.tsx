...
export default function EntradaNF() {
  ...
  const handleRegistrar = async () => {
    if (!numeroNf || !fornecedor) { toast.error("Preencha Nº NF e Fornecedor"); return; }
    if (rolos.length === 0) { toast.error("Adicione ao menos um rolo de tecido"); return; }
    setSalvando(true);
    try {
  ...
  const handleNovaEntrada = () => {
    setNumeroNf(""); setFornecedor(""); setValorTotal(0);
    setDataEntrada(new Date().toISOString().split("T")[0]);
    setRolos([]); setNovoRolo({ ...emptyRolo }); setSalvo(false);
  };
