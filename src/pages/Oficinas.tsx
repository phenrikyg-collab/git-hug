...
export default function Oficinas() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editId) { await updateMut.mutateAsync({ id: editId, ...payload }); toast.success("Oficina atualizada!"); }
      else { await createMut.mutateAsync(payload); toast.success("Oficina criada!"); }
      setOpen(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-serif font-bold text-foreground"><span className="text-primary">Oficinas</span></h1>
