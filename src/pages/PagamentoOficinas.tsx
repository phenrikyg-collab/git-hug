...
export default function PagamentoOficinas() {
  ...
  const totalPendente = ordensExternas.filter((o) => o.pagamento_oficina_status !== "Pago").reduce((sum, o) => sum + getValorFinal(o), 0);
  const totalPago = ordensExternas.filter((o) => o.pagamento_oficina_status === "Pago").reduce((sum, o) => sum + getValorFinal(o), 0);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-serif font-bold text-foreground">Pagamento de <span className="text-primary">Oficinas</span></h1>
        <p className="text-sm text-muted-foreground mt-1">Controle de pagamentos das oficinas externas</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="border-l-4 border-l-warning">
            <CardContent className="pt-4 pb-4">
              <div className="flex items-center justify-between">
                <div><p className="text-xs text-muted-foreground uppercase tracking-wide font-medium">A Pagar</p><p className="text-2xl font-bold text-warning">R$ {totalPendente.toFixed(2)}</p></div>
                <div className="p-2 rounded-full bg-warning/10"><AlertCircle className="h-5 w-5 text-warning" /></div>
              </div>
            </CardContent>
          </Card>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <Card className="border-l-4 border-l-success">
            <CardContent className="pt-4 pb-4">
              <div className="flex items-center justify-between">
                <div><p className="text-xs text-muted-foreground uppercase tracking-wide font-medium">Total Pago</p><p className="text-2xl font-bold text-success">R$ {totalPago.toFixed(2)}</p></div>
                <div className="p-2 rounded-full bg-success/10"><CheckCircle className="h-5 w-5 text-success" /></div>
              </div>
            </CardContent>
          </Card>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Card className="border-l-4 border-l-primary">
            <CardContent className="pt-4 pb-4">
              <div className="flex items-center justify-between">
                <div><p className="text-xs text-muted-foreground uppercase tracking-wide font-medium">Ordens Externas</p><p className="text-2xl font-bold text-foreground">{totalOrdens}</p></div>
                <div className="p-2 rounded-full bg-primary/10"><Factory className="h-5 w-5 text-primary" /></div>
              </div>
            </CardContent>
          </Card>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          <Card className="border-l-4 border-l-danger">
            <CardContent className="pt-4 pb-4">
              <div className="flex items-center justify-between">
                <div><p className="text-xs text-muted-foreground uppercase tracking-wide font-medium">Pendentes</p><p className="text-2xl font-bold text-danger">{ordensPendentes}</p></div>
                <div className="p-2 rounded-full bg-danger/10"><Clock className="h-5 w-5 text-danger" /></div>
              </div>
            </CardContent>
          </Card>
        </motion.div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="space-y-1">
              ...
              <Select value={filtroOficina} onValueChange={setFiltroOficina}>
                <SelectTrigger className="w-[200px] h-9"><SelectValue placeholder="Todas" /></SelectTrigger>
                <SelectContent>
                ...
                {ordensExternas.map((o) => {
                  ...
                  return (
                    <TableRow key={o.id} className={isPago ? "opacity-60" : ""}>
                      ...
                      <TableCell><span className="text-sm font-medium">{oficina?.nome_oficina ?? "—"}</span></TableCell>
                      <TableCell className="text-right">{qty}</TableCell>
                      <TableCell className="text-right text-muted-foreground">R$ {custoPorPeca.toFixed(2)}</TableCell>
                      <TableCell className="text-right">
                        {isPago ? (
                          <span className="font-semibold">R$ {getValorFinal(o).toFixed(2)}</span>
                        ) : (
                          <Input type="number" step="0.01" className="w-28 h-8 text-right text-sm font-semibold ml-auto"
                            value={valoresEditados[o.id] !== undefined ? valoresEditados[o.id] : totalBase}
                            onChange={(e) => setValoresEditados((prev) => ({ ...prev, [o.id]: Number(e.target.value) }))} />
                        )}
                      </TableCell>
                      <TableCell className="text-muted-foreground text-sm">{formatDateBR(o.data_inicio)}</TableCell>
                      <TableCell className="text-muted-foreground text-sm">{formatDateBR(o.data_fim)}</TableCell>
                      <TableCell className="text-right">
                        {dias !== null ? (<Badge variant={dias > 10 ? "destructive" : "secondary"} className="text-xs">{dias}d</Badge>) : "—"}
                      </TableCell>
                      <TableCell><StatusBadge status={o.status_ordem ?? ""} /></TableCell>
                      <TableCell>
                        {isPago ? (
                          <Badge className="bg-success/15 text-success border-success/30 text-xs gap-1"><CheckCircle className="h-3 w-3" /> Pago</Badge>
                        ) : (
                          <Badge variant="outline" className="text-warning border-warning/30 text-xs">Pendente</Badge>
                        )}
                      </TableCell>
                      <TableCell>
                        {!isPago && (
                          <Button size="sm" variant="outline" className="gap-1.5 text-xs h-8 border-success/30 text-success hover:bg-success/10 hover:text-success"
                            onClick={() => handleMarcarPago(o)} disabled={updateOP.isPending}>
